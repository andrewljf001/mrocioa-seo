...
Release 1.0.1 正式源码：顶部状态栏 + 多串口会话、左侧连接配置栏、中央数据监视终端、底部发送区、右侧可切换波形图与协议解析面板。

规范版本号：`1.0.1`。首次正式发行的历史显示名 `Release 1.0` 在正式记录中规范为 `1.0.0`。
...
本次发行范围与验收记录在 [NEXT-RELEASE.md](NEXT-RELEASE.md)。`CHANGELOG.md` 只记录已经正式发行的版本。
